...
void HierarchyScene::setMode(Mode mode)
{
    mode_ = mode;
}

void HierarchyScene::setItemType(AnfNodeItem::NodeType type)
{
    itemType_ = type;
}
...
void HierarchyScene::mouseReleaseEvent(QGraphicsSceneMouseEvent *mouseEvent)
{
    if (line_ != nullptr && mode_ == InsertLine) {
        ...
        if (startItems.count() > 0 && endItems.count() > 0 &&
            startItems.first()->type() == AnfNodeItem::Type &&
            endItems.first()->type() == AnfNodeItem::Type &&
