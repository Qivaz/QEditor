...
QStringList FileEncoding::encodingNames() {
    return encodingNameList_;
}
...
QString FileEncoding::ProcessAnsi(QFile &file)
{
    ...
    // Use System.
    auto systemCodec = QTextCodec::codecForLocale();  // QTextCodec::codecForName("System"); // QTextCodec::codecForMib(0); // Use system codec.
    ...
    for (const auto &name : encodingNames()) {
        ...
        qDebug() << "codec: " << codec->name() << ", invalidChars: " << check_state.invalidChars << ", minInvalidCharsNum: " << minInvalidCharsNum
                    << ", text: " << ansiText.mid(0, 15) << "..., bytearry: " << data.mid(0, 15).data() << "...";
